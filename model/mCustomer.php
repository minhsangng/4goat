<?php
    class mCustomer {
        ...
        public function mInsertCustomer($customerName, $phoneNumber, $email, $loginName, $password, $address, $customerType) {
            $db = new tmdt();
            $conn = $db->connect();
            $sql = "INSERT IGNORE INTO customer (customerName, phoneNumber, email, loginName, password, address, customerType) VALUE ('$customerName', '$phoneNumber', '$email', '$loginName', '$password', '$address', $customerType)";
            $result = $conn->query($sql);
